...
| **Batch Normalization** | Normalize activations per mini-batch | Enables higher learning rates, faster training, acts as regularizer. |
...
Batch Normalization (BatchNorm) is a technique that normalizes the inputs to each layer, making neural networks faster, more stable, and easier to train.

### The Problem: Internal Covariate Shift

As training progresses, the distribution of inputs to each layer changes because the parameters of previous layers change. This phenomenon is called **internal covariate shift** and forces each layer to continuously adapt to new input distributions.

```
Without Batch Norm:
Layer 1 output → changes → Layer 2 must adapt
Layer 2 output → changes → Layer 3 must adapt
... (cascading instability)

With Batch Norm:
Layer 1 output → normalized → stable input to Layer 2
Layer 2 output → normalized → stable input to Layer 3
... (stable training)
```

### How Batch Normalization Works

For each mini-batch, BatchNorm normalizes the pre-activation values (Z) to have zero mean and unit variance, then scales and shifts using learnable parameters:
...
2. Normalize:
   Z_norm = (Z - μ) / √(σ² + ε)   # ε prevents division by zero

3. Scale and shift (learnable parameters γ and β):
   Z_out = γ * Z_norm + β
```

**Why learnable γ and β?** The network can learn to undo the normalization if needed. If γ = σ and β = μ, BatchNorm becomes an identity function. This gives the network flexibility.

### Benefits of Batch Normalization

| Benefit | Explanation |
|---------|-------------|
| **Faster Training** | Enables higher learning rates without divergence |
| **Reduces Sensitivity to Initialization** | Less dependent on careful weight initialization |
| **Acts as Regularization** | Noise from batch statistics provides regularization effect |
| **Reduces Internal Covariate Shift** | Stabilizes the distribution of layer inputs |
| **Allows Deeper Networks** | Easier to train very deep networks |
...
**During Inference:**
- Use running mean and variance (computed during training)
- No batch statistics needed (can process single examples)

```python
# Running average update (during training)
...
nn = FiveLayerNN(
    layer_dims,
    learning_rate=0.01,        # Can use higher LR with BatchNorm
    initialization='he',
    use_batch_norm=True,       # Enable batch normalization
    bn_momentum=0.9            # Momentum for running statistics
)

# Train with mini-batches (BatchNorm works best with mini-batches)
losses = nn.train(X_train, Y_train, epochs=2500, mini_batch_size=64)
```

### BatchNorm + Other Techniques

| Combination | Recommendation |
|-------------|----------------|
| BatchNorm + Higher LR | Use 2-10x higher learning rate |
| BatchNorm + L2 | Reduce L2 strength (BatchNorm provides regularization) |
| BatchNorm + Dropout | Can reduce dropout rate (BatchNorm regularizes) |
| BatchNorm + Adam | Excellent combination for fast, stable training |

### Comparison Results (Example)

```
Configuration             Train Acc    Test Acc     Final Loss
--------------------------------------------------------------
No BatchNorm              93.00        72.00        0.200221
With BatchNorm            95.50        76.00        0.142341
BatchNorm (higher LR)     96.00        77.00        0.125678
BatchNorm + L2            94.50        78.00        0.156234
BatchNorm + Adam          97.13        80.00        0.098765    (best!)
```

### Where to Apply BatchNorm

BatchNorm is applied **after the linear transformation and before the activation function**:

```
Without BatchNorm:  Z = W*A + b  →  A = ReLU(Z)
With BatchNorm:     Z = W*A + b  →  Z_bn = BatchNorm(Z)  →  A = ReLU(Z_bn)
```

**Note:** When using BatchNorm, the bias term `b` is often redundant (BatchNorm's β serves the same purpose), but we keep it for simplicity in this implementation.
...
### 12. Batch Normalization Comparison (`batch_norm_comparison.png`)

Training cost curves comparing models with and without batch normalization, including combinations with other techniques.

### 13. Learning Rate Decay Comparison (`learning_rate_decay_comparison.png`)
...
| `batch_norm_comparison.png` | Cost curves for models with/without batch normalization |
...
## Neural Network Tuning Techniques

This section provides a comprehensive guide to all the tuning techniques available for improving neural network performance.

### Overview of Tuning Categories
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  1. DATA PREPROCESSING          2. ARCHITECTURE DESIGN                  │
│     • Input normalization          • Layer depth (# layers)             │
│     • Feature scaling              • Layer width (neurons/layer)        │
│     • Data augmentation            • Activation functions               │
│                                                                         │
│  3. WEIGHT INITIALIZATION       4. REGULARIZATION                       │
│     • He (for ReLU)                • L2 weight decay (λ)                │
│     • Xavier (for tanh/sigmoid)    • Dropout (keep_prob)                │
│     • Random, Zeros (avoid)        • Batch normalization                │
│                                    • Early stopping                     │
│                                                                         │
│  5. OPTIMIZATION                6. LEARNING RATE STRATEGIES             │
│     • Gradient Descent             • Fixed learning rate                │
│     • Momentum                     • Learning rate decay                │
│     • RMSprop                      • Scheduled (step) decay             │
│     • Adam (recommended)           • Warm restarts                      │
│                                                                         │
│  7. BATCH SIZE                  8. TRAINING DURATION                    │
│     • Full batch (slow, smooth)    • Number of epochs                   │
│     • Mini-batch (balanced)        • Early stopping criteria            │
│     • SGD (fast, noisy)            • Convergence monitoring             │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
|-----------|---------|-------------|--------|
| **Z-Score** | `(X - mean) / std` | General use, default choice | High - stabilizes training |
| **Min-Max** | `(X - min) / (max - min)` | Bounded features, pixels | Medium - good for images |
| **Mean Centering** | `X - mean` | Simple preprocessing | Low - basic improvement |
| **L2 Normalization** | `X / ||X||` | Direction matters (similarity) | Context-dependent |

**Best Practice:** Always normalize using training set statistics, then apply same transformation to test set.

### 2. Architecture Design Choices

| Choice | Options | Trade-offs |
|--------|---------|------------|
| **Depth** | Shallow (2-3) vs Deep (5+) | Deeper = more expressive but harder to train |
| **Width** | Narrow vs Wide layers | Wider = more capacity but more parameters |
| **Activation** | ReLU, tanh, sigmoid, Leaky ReLU | ReLU is default; use others for specific cases |
| **Output** | Sigmoid (binary), Softmax (multi-class) | Match to problem type |

**Funnel Architecture (Used Here):**
```
[12288] → [20] → [7] → [5] → [3] → [1]
   ↓         ↓       ↓       ↓       ↓
Raw Data  Features  Abstract  Decision  Output
```
...
|--------|---------|----------|------------|
| **He** | `randn * sqrt(2/n_prev)` | ReLU activations | Using tanh/sigmoid |
| **Xavier** | `randn * sqrt(1/n_prev)` | tanh/sigmoid | Using ReLU |
| **Random (large)** | `randn * 10` | Never | Always avoid |
| **Zeros** | `0` | Never | Always avoid |

**Why It Matters:**
- Bad initialization → vanishing/exploding gradients → training fails
- He initialization compensates for ReLU "killing" half the neurons

### 4. Regularization Techniques Comparison

| Technique | Parameter | Effect | When to Use |
|-----------|-----------|--------|-------------|
| **L2 Regularization** | λ (0.01-1.0) | Penalizes large weights | Always (light L2 rarely hurts) |
| **Dropout** | keep_prob (0.5-0.9) | Random neuron deactivation | Overfitting with large networks |
| **Batch Normalization** | momentum (0.9-0.99) | Normalizes activations | Almost always beneficial |
| **Early Stopping** | patience epochs | Stops when val loss plateaus | When overfitting is detected |
| **Data Augmentation** | transforms | Increases effective data size | Limited training data |
...
Underfitting ←──────────────────────────────────→ Overfitting
              ...
              Light Reg.    Moderate    Strong Reg.
L2 (λ):       0.001-0.01    0.01-0.1    0.1-1.0
Dropout:      0.9-0.95      0.8-0.9     0.5-0.8
BatchNorm:    Always helps (mild regularization effect)
```

### 5. Optimization Algorithm Selection

| Algorithm | Learning Rate | Best For | Hyperparameters |
|-----------|---------------|----------|-----------------|
| **SGD** | 0.01-0.1 | Simple problems, fine control | Just LR |
| **Momentum** | 0.01-0.1 | Faster than SGD, smoother | LR, β (0.9) |
| **RMSprop** | 0.001-0.01 | Adaptive, non-stationary | LR, β₂ (0.999) |
| **Adam** | 0.0001-0.01 | **Default choice**, robust | LR, β₁ (0.9), β₂ (0.999) |
...
Works well? → Done
    ↓ No
Try SGD + Momentum (may generalize better)
    ↓
Still issues? → Tune learning rate first
```
...
|----------|---------|-------------|
| **Fixed** | `lr = constant` | Simple problems, short training |
| **Continuous Decay** | `lr = lr₀ / (1 + decay * epoch)` | Smooth reduction |
| **Step Decay** | `lr = lr₀ / (1 + decay * floor(epoch/interval))` | More control, common |
| **Exponential** | `lr = lr₀ * decay^epoch` | Aggressive reduction |
| **Cosine Annealing** | `lr = lr_min + 0.5*(lr_max-lr_min)*(1+cos(π*t/T))` | State-of-the-art |

**Learning Rate Tuning Process:**
1. Start with default (0.01 for SGD, 0.001 for Adam)
2. If loss oscillates/diverges → decrease by 10x
3. If loss decreases very slowly → increase by 10x
4. Fine-tune around best value

### 7. Batch Size Selection

| Batch Size | Name | Gradient Quality | Speed | Memory |
|------------|------|------------------|-------|--------|
| 1 | SGD | Very noisy | Fast iterations | Minimal |
| 32-64 | Mini-batch | Good balance | Good | Moderate |
| 128-256 | Large mini-batch | Smoother | Faster (GPU) | Higher |
| Full dataset | Batch GD | Exact | Slow iterations | Maximum |

**Recommendation:** Start with 32 or 64, increase if GPU memory allows.

### 8. Training Duration & Early Stopping

**Monitoring Metrics:**
- Training loss (should decrease)
- Validation loss (should decrease, then may plateau or increase)
- Train-validation gap (indicates overfitting)
...
When tuning a neural network, follow this priority order:

| Priority | Technique | Impact | Effort |
|----------|-----------|--------|--------|
| 1 | **Data normalization** | High | Low |
| 2 | **Proper initialization** | High | Low |
| 3 | **Learning rate** | Very High | Medium |
| 4 | **Batch normalization** | High | Low |
| 5 | **Optimizer choice** | Medium-High | Low |
| 6 | **Regularization (L2/Dropout)** | Medium | Medium |
| 7 | **Architecture (depth/width)** | Medium | High |
| 8 | **Learning rate decay** | Low-Medium | Low |
| 9 | **Batch size** | Low | Low |
...
□ 3. Start with Adam optimizer (lr=0.001)
□ 4. Enable batch normalization
□ 5. Add light L2 regularization (λ=0.01-0.1)
□ 6. Use mini-batches (32-64)
□ 7. Monitor train vs validation gap
□ 8. Add dropout if overfitting (keep_prob=0.8-0.9)
□ 9. Implement early stopping
□ 10. Fine-tune learning rate if needed
```
...
3. **Batch normalization accelerates training** - Enables higher learning rates, provides implicit regularization
4. **Regularization reduces overfitting** - L2 penalizes large weights, dropout prevents co-adaptation
5. **Combine techniques** - BatchNorm + L2 + dropout together often gives best generalization
6. **Learning rate** is crucial - too high causes instability, too low is slow
7. **Adam optimizer** is generally the best choice - combines momentum + RMSprop with less tuning
8. **Mini-batches** (32-64) offer the best balance of speed and stability
9. **Learning rate decay** helps fine-tune convergence - scheduled decay gives more control than continuous
10. **F1 score** is better than accuracy for imbalanced datasets
11. **Monitor the gap** - Large train-test accuracy gap indicates overfitting
12. **Gradient checking** verifies backpropagation correctness before training (runs automatically in five_layer_nn.py)
...
# 3. Create model with best settings (including batch normalization)
nn = FiveLayerNN(
    layer_dims,
    learning_rate=0.001,        # Lower LR for Adam
    initialization='he',        # Best for ReLU
    ...
    use_batch_norm=True,        # Enable batch normalization
    bn_momentum=0.9,            # BatchNorm momentum for running stats
    lambd=0.1,                  # Light L2 regularization
    keep_prob=0.9,              # 10% dropout (less needed with BatchNorm)
...
| **Batch Normalization** | Technique that normalizes layer inputs to have zero mean and unit variance, with learnable scale (γ) and shift (β). |
...
| **Internal Covariate Shift** | Change in distribution of layer inputs during training as parameters of previous layers change. BatchNorm addresses this. |
